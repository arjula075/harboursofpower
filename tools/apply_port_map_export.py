#!/usr/bin/env python3
"""Apply data/port_map_editor_export.json edits into data/world_full.json (map_u / map_v only).

  python3 tools/apply_port_map_export.py
  python3 tools/apply_port_map_export.py --dry-run
"""
...
EXPORT = REPO / "data" / "port_map_editor_export.json"
...
def main() -> None:
    ...
    if not args.keep_export:
        applied = EXPORT.with_name("port_map_editor_export.applied.json")
        EXPORT.rename(applied)
        print(f"Renamed {EXPORT.name} → {applied.name} (editor loads world_full only)")
